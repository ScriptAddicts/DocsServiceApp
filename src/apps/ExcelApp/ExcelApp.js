  ...
  ExcelApp = function () {
    ...
    sheetNameToFileName = function () {
      ...
      if (this.mainObj.fileObj.hasOwnProperty(this.workbook)) {
        ...
        sheets = rootChildren
          .getChild("sheets", rootChildren.getNamespace())
          .getChildren();
        return (this.mainObj.sheetsObj = sheets.reduce(
          (o, e) => {
        ...
        return putInternalError.call(
          this,
          "This Excel data cannot be analyzed."
        );
      }
    };
      ...
      if (this.mainObj.fileObj.hasOwnProperty(this.styles)) {
        ...
        if (numFmtsC) {
          ...
          numFmtsObj = numFmts.reduce((o, e) => {
            return Object.assign(o, {
              [e.getAttribute("numFmtId").getValue()]: e
                .getAttribute("formatCode")
                .getValue(),
            });
          }, {});
        }
    ...
    getValuesAsObject = function () {
      ...
      if (this.mainObj.sheetsObj.sheetsObj.hasOwnProperty(this.obj.sheetName)) {
        checkFilename = this.mainObj.sheetsObj.sheetsObj[this.obj.sheetName]
          .sheet;
        if (this.mainObj.fileObj.hasOwnProperty(checkFilename)) {
          xmlObj = getXmlObj.call(this, checkFilename);
          root = xmlObj.getRootElement();
          sheetData = root
            .getChild("sheetData", root.getNamespace())
            .getChildren();
          rows = [];
          sheetData.forEach((s) => {
            var cols, rowNum;
    ...
    getImagesAsObject = function () {
      ...
      if (
        !this.mainObj.sheetsObj.sheetsObj.hasOwnProperty(this.obj.sheetName)
      ) {
        putError.call(this, "No sheet name.");
      }
      drawingFilename = this.mainObj.sheetsObj.sheetsObj[
        this.obj.sheetName
      ].sheet
        .replace("worksheets", "drawings")
        .replace("sheet", "drawing");
      if (this.mainObj.fileObj.hasOwnProperty(drawingFilename)) {
        ...
        imageObj = rootChildren.map((e) => {
          ...
          temp.range.a1Notation =
            columnToLetter.call(this, temp.range.col) + temp.range.row;
          ...
          if (pic) {
            ...
            temp.image.blob =
              this.mainObj.fileObj[filename].setName(
                filename.split("/").pop()
              ) || null;
            temp.image.innerCell =
              temp.range.colOff === 0 && temp.range.rowOff === 0 ? true : false;
          } else if (sp) {
            temp.drawing = {
              message:
                "In the current stage, the object of drawing cannot be directly retrieved as the blob.",
            };
          }
        ...
        return putInternalError.call(
          this,
          "Internal error: Image files cannot be retrieved."
        );
      }
    };
    ...
    getCommentsAsObject = function () {
      ...
      if (this.mainObj.sheetsObj.sheetsObj.hasOwnProperty(this.obj.sheetName)) {
        ...
        if (this.mainObj.fileObj.hasOwnProperty(checkFilename)) {
          ...
          rootChildren = xmlObj.getRootElement().getChildren();
          for (
            i = j = 0, ref1 = rootChildren.length;
      ...
      if (f.length > 0) {
        ...
        root = xmlObj.getRootElement();
        commentList = root
          .getChild("commentList", root.getNamespace())
          .getChildren();
        comments = commentList.reduce((ar, e) => {
          ...
          if (ref) {
            ...
            if (t) {
              ...
              commentObj = [...tempComment.matchAll(/\t\-\w.+/g)]
                .map(([h]) => {
                  return h.replace(/\t-/, "");
                })
                .map((h, l) => {
